...
- Result persistence (writing a finished `BatchSimulationResult` to Azure SQL) lives in its own shared project, `BlackjackStreakSimulator.Data`. The Azure Functions app references it directly to write a row once a batch finishes; the Blazor app references it directly to read run history back for display. Same reasoning as the engine — no swappable interface, just shared code, two hosts using it for different sides of the same job. Deliberately **not** a second Azure Function reached over HTTP for either side — that would be an unnecessary network hop, when Blazor is already server-side .NET with direct SQL access. Step-through mode never persists anything — it's a single Run, shown live and then gone; only completed batches land in run history.
