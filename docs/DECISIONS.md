...
"Always-Ready" instances (a fixed number of pre-warmed instances, configured under Scale and Concurrency) remove per-request cold-start variance, and per the timing above, more instance memory buys real, consistent throughput on top of that. But Flex Consumption's Always-Ready billing has **no free monthly grant** (unlike its own On-Demand executions, which do) — it's a flat ongoing cost regardless of actual traffic, so the memory tier behind it matters a lot for a low-traffic personal demo: 2048MB continuous ≈ $20.74/mo at the Always-Ready Baseline rate, versus 512MB continuous ≈ $5.18/mo.

Kept **one Always-Ready instance at 512MB** — the cheaper end of that tradeoff. Consistency (no cold-start variance) was worth the small always-on cost, but the extra throughput 2048MB buys (~4.8ms/run vs. 512MB's ~21ms/run) wasn't worth roughly 4x the monthly cost on top of it, since neither memory tier changes the real ceiling on a large batch anyway — that's `HttpClient.Timeout`, not Function throughput. See "Why the batch size stays capped by UX messaging" below.
...
Once the Flex Consumption memory/throughput tradeoff above was understood, the next question was whether to invest further — a higher-memory Always-Ready instance, or migrating the Function App to a Dedicated App Service Plan — to support genuinely large batches (100,000+ runs) without timing out.
...
Resolution: kept the Function App at 1 Always-Ready instance / 512MB (cheap and consistent, not fast) and `HttpClient.Timeout` at its unconfigured 100-second default, and turned the resulting constraint into intentional, transparent UX on the Batch Simulation page — a collapsed "why might this time out" explainer, a live-updating stopwatch visible while a batch runs (so the 100-second approach is something the user can actually watch happen), and a specific friendly error message on timeout suggesting 1,000-3,000 runs instead of a generic failure. If demand for larger batches ever becomes real (e.g. the project gets outside traffic beyond its original purpose), that's the trigger to revisit a higher memory tier or a Dedicated plan — not a default reached for now.
...
- **Azure Functions (Flex Consumption)** — ~$5/mo flat, from 1 Always-Ready instance kept at the cheapest memory tier (512MB) specifically to keep that continuous cost low — Always-Ready billing has no free grant, so a higher memory tier (2048MB) would have meant ~$20.74/mo instead for throughput this project doesn't need enough of to justify the extra cost.
